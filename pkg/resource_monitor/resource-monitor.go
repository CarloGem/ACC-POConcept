...
import (
	...
	log "sigs.k8s.io/kueue/cmd/experimental/demo-acc/pkg/logger"
)
...
func (sm *SnapshotManager) UpdateSnapshot(clientset *kubernetes.Clientset, namespace, nodeName string) {
	logger := log.GetLogger().WithField("component", "resource-monitor")
	...
	if err != nil {
		logger.WithError(err).Error("Error listing pods in namespace %s", namespace)
		return
	}
	...
	for _, node := range nodes.Items {
		...
		if nodeName == "" || node.Name == nodeName {
			...
			if len(allocatable) == 0 { // Ensuring that allocatable has a valid value
				logger.Warnf("RESOURCE-MONITOR: Node %s has no allocatable resources defined; skipping", node.Name)
				continue
			}
